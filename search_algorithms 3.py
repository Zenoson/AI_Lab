...
class TreeSearchAlgorithm(GoalSearchAgent):
    ...
    def search(self, 
        ...
        while (len(self.frontier)>0):
            ...
            for i in actions:
                next_state = state.get_next_state(i)
            ...
            if next_state != state.parent:
                ...
                print("action ignored, leads back to parent")
...
class UniformCostSearch(GoalSearchAgent):
    ...
    The heapq library allows you to use a list as an efficient heap.
    """
    ...
    def enqueue(self, state: StateNode, cutoff: Union[int, float] = INF):
        ...
        if state.path_cost <= cutoff: 
            ...
            self.total_enqueues += 1  # not sure if this one works
